  ...
  switch (type) {
    ...
    case 'positiveInteger':
      ...
      if (
        ...
        throw createParseError({
          message: `Expected ${type === 'id' || type === 'positiveInteger' ? 'a positive integer number' : 'an integer number'}`,
          path,
    ...
    case 'positiveNumber':
      ...
      if (String(value) !== nonParsedValue) {
        throw createParseError({
          message: `Expected ${type === 'positiveNumber' ? 'a positive number' : 'a number'}`,
          path,
          value: nonParsedValue
        });
      }
    ...
    case 'boolean':
      ...
      switch (value) {
        ...
        default:
          throw createParseError({
            message: `Expected either "1" or "✓" for TRUE or "0" for FALSE but received "${nonParsedValue}"`,
            path,
            value: nonParsedValue
          });
      }
