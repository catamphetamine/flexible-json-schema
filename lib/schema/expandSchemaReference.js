...
export default function expandSchemaReference(schemaEntry, { path, context }) {
  ...
  if (typeof schemaEntry.extends === 'string') {
    ...
    if (!isObject(extendsSchemaEntry.schema)) {
      throw new SchemaError(`A schema can only extend another schema that describes an object's properties. It can't extend \`arrayOf\` schema, \`oneOfType\` schema, simple value \`type\` schema, etc.`, {
