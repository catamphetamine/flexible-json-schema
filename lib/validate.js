...
export default function schemaValidation(schema, {
  ...
  const context = {
    ...
    dateStrings,
    dateFormat,
    // `customTypes` parameter could be used to avoid "polluting" the global scope
    // with custom types in case of using `useCustomTypes()` function.
    customTypes,
  ...
  return function validate(input) {
    ...
    if (convertDates) {
      const parse = schemaParser(schema, {
        ...
        customTypes: context.customTypes,
...
function compileSchemaEntry(schemaEntry, {
  ...
  const markAsRequiredOrOptional = (propertyYupSchema) => {
    return markAsRequiredOrOptional_(
      ...
      isInlineObjectSchema(schemaEntry) ? undefined : schemaEntry.nullable,
      { schemaPath, context }
    );
  }
