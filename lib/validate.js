...
function isInlineObjectSchema(schemaEntry) {
  const {
    oneOf,
    arrayOf,
    objectOf,
  ...
  if (oneOf || arrayOf || objectOf || oneOfType || typeof type === 'string' || typeof schema === 'string' || isObject(schema)) {
    return false;
  }
  return true;
}
...
function compileSchemaEntry(schemaEntry, {
  ...
  const validateDescription = () => {
    if (!description) {
      throw new SchemaError(`"description" is missing in schema entry${key ? ' for key "' + key + '"' : ''}:\n${JSON.stringify(schemaEntry, null, 2)}`, {
        path
      });
    }
  };
