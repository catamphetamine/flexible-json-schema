...
export default function schemaValidation(schema, {
  ...
  const yupSchema = expandSchemaProperty(schema, {}, {
    strict: true,
    schemas,
    allowEmptyStrings,
    allowEmptyArrays,
...
function expandSchemaProperty(schemaEntry, {
  key,
...
function defineNestedObjectSchema(schemaEntry, {
  ...
  for (const key of Object.keys(schemaEntry)) {
    shape[key] = expandSchemaProperty(schemaEntry[key], {
      key
    }, context);
  }
