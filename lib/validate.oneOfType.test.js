...
describe('validate/oneOfType', function() {
  ...
  it('should match `isNot` ("$isNot")', function() {
    const schema = {
      anObject: {
        description: 'Object',
        oneOfType: [
    ...
    }).to.throw('a must be a `string` type')
  })

  it('should detect obvious cases of ambiguity at schema compile time', function() {
    const schema = {
      anObject: {
        description: 'Object',
        oneOfType: [
  ...
  it('should validate `oneOfType` ($or + $exists)', function() {
    const schema = {
      description: 'One of type',
      oneOfType: [{
        is: 'object',
    ...
    expect(() => validate({ a: 'a' })).to.throw('unsupported');
  })
})
